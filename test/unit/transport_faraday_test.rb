        ...
        class FaradayTest < Minitest::Test
          ...
          context 'Faraday transport' do
            ...
            should 'properly prepare the request' do
              @transport.connections.first.connection.expects(:run_request).with do |method, _url, body, headers|
            ...
            should 'properly pass the Content-Type header option' do
              transport = Faraday.new(
                ...
                options: { transport_options: { headers: { 'Content-Type' => 'foo/bar' } } }
              )

              transport.connections.first.connection.expects(:run_request).with do |_method, _url, _body, headers|
            ...
            should 'merge in parameters defined in the Faraday connection parameters' do
              ...
              transport.connections.first.connection.expects(:run_request).with do |_method, url, _params, _body|
            ...
            should 'override options credentials if passed explicitly' do
              transport = Faraday.new(
                hosts: [
                  { host: 'foobar', port: 1234, user: 'foo', password: 'bar' },
            ...
            should 'override options scheme if passed explicitly' do
              transport = Faraday.new(
                hosts: [{ host: 'foobar', port: 1234, scheme: 'http' }],
                options: { scheme: 'https' }
              )
